...
class SimpleNet(torch.nn.Module):
    ...
    def load(
        ...
        self.discriminator = KAN([1536, 512, 64, 1])
        print('_____discriminator______>', self.discriminator)
    ...
    def train(self, training_data, test_data):
        ...
        if os.path.exists(ckpt_path):
            ...
            scores, segmentations, features, labels_gt, masks_gt = self.predict(test_data)
            auroc, full_pixel_auroc, anomaly_pixel_auroc, acc = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt)
            return auroc, full_pixel_auroc, anomaly_pixel_auroc, acc
        ...
        for i_mepoch in range(self.meta_epochs):
            ...
            scores, segmentations, features, labels_gt, masks_gt = self.predict(test_data)
            auroc, full_pixel_auroc, pro, acc = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt)
            ...
            self.logger.logger.add_scalar("acc", acc, i_mepoch)

            if best_record is None:
                best_record = [auroc, full_pixel_auroc, pro, acc]
            ...
            print(f"----- {i_mepoch} I-AUROC:{round(auroc, 4)}(MAX:{round(best_record[0], 4)})"
                  ...
                  f"  Acc:{round(acc, 4)}(MAX:{round(best_record[3], 4)}) -----"
                  )
    ...
    def _train_discriminator(self, input_data):
        ...
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(self.gan_epochs):
                ...
                # if self.cos_lr: self.dsc_schl.step()
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():   # [20, 3, 288, 288] ---> [25920, 1536]

...
class SimpleNet(torch.nn.Module):
    ...
    def _evaluate(self, test_data, scores, segmentations, labels_gt, masks_gt):
        ...
        # print('________scores_________>', scores)
        # print('________labels_gt_________>', labels_gt)
        auroc = metrics.compute_imagewise_retrieval_metrics(scores, labels_gt)["auroc"]

        # 手动计算准确率
        scores_pre_gt = np.where(scores > 0, 1, 0)
        pre_correct_cnt = sum(a == b for a, b in zip(scores_pre_gt, labels_gt))
        acc = pre_correct_cnt * 1.0 / len(labels_gt)
        ...
        return auroc, full_pixel_auroc, pro, acc
    ...
    def train(self, training_data, test_data):
        ...
        if os.path.exists(ckpt_path):
            ...
            auroc, full_pixel_auroc, anomaly_pixel_auroc, acc = self._evaluate(test_data, scores, segmentations, labels_gt, masks_gt)
        ...
        for i_mepoch in range(self.meta_epochs):
            ...
            scores, segmentations, labels_gt, masks_gt = self.predict(test_data)
            auroc, full_pixel_auroc, pro, acc = self._evaluate(test_data, scores, segmentations, labels_gt, masks_gt)
        ...
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    self.dsc_opt.zero_grad()    # 清零判别器的梯度，准备进行反向传播
                    if self.pre_proj > 0:
                        self.proj_opt.zero_grad()   # 将图像数据转换为浮点型张量，并将其移动到指定的设备上
                    ...
                    if self.pre_proj > 0:
                        embed_tuple = self._embed(img, evaluation=False)
                        true_feats = self.pre_projection(embed_tuple[0])     # 使用模型的 _embed 方法对图像进行特征提取，得到真实图像的特征表示 true_feats
                    else:
                        true_feats = self._embed(img, evaluation=False)[0]
                    
                    noise_idxs = torch.randint(0, self.mix_noise, torch.Size([true_feats.shape[0]]))    # 生成随机噪音的索引
                    noise_one_hot = torch.nn.functional.one_hot(noise_idxs, num_classes=self.mix_noise).to(self.device) # (N, K) 将整数张量 noise_idxs 转换为相应的独热编码张量
                    ...
                    for k in range(self.mix_noise):
                        noise_std_k = self.noise_std * 1.1 ** (k)
                        torch_normal_t = torch.normal(0, noise_std_k, true_feats.shape)     # 生成正态分布的噪音张量
                        noise_normal_list.append(torch_normal_t)
                    noise = torch.stack(noise_normal_list, dim=1).to(self.device)

                    # 根据独热编码将不同噪音叠加到真实特征表示上
                    # noise = (noise * noise_one_hot.unsqueeze(-1)).sum(1)
                    un_noise_one_hot = noise_one_hot.unsqueeze(-1)
                    torch_mm_t = (noise * un_noise_one_hot)   # [N K C] * [N 1 1]
                    noise = torch_mm_t.sum(1)

                    # 将生成的噪音添加到真实特征表示上，得到虚假（合成）特征表示
                    fake_feats = true_feats + noise

                    # 使用判别器对真实特征表示和虚假特征表示进行评分
                    tensor_cat = torch.cat([true_feats, fake_feats])
                    scores = self.discriminator(tensor_cat)

                    # 真实样本和虚假样本的得分
                    true_scores = scores[:len(true_feats)]
                    fake_scores = scores[len(fake_feats):]
                    
                    th = self.dsc_margin    # th，这个值代表了判别器的边界值（阈值）
                    '''
                        true_scores.detach() >= th: 通过对真实样本的分数应用阈值，得到一个布尔张量，表示哪些真实样本的分数超过了阈值
                        (true_scores.detach() >= th).sum() / len(true_scores) 计算了真实样本中分数超过阈值的样本所占的比例，这个比例被称为真实样本的置信度
                    '''
                    p_true = (true_scores.detach() >= th).sum() / len(true_scores)
                    p_fake = (fake_scores.detach() < -th).sum() / len(fake_scores)

                    '''  计算了真实样本的损失。首先，通过将阈值 th 添加到真实样本的分数上，然后应用了裁剪操作，将小于零的值设置为零 '''
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():
            ...
            # 使用鉴别器模型对提取的特征进行评分，分数为负值，因为通常鉴别器输出的是真实样本的得分

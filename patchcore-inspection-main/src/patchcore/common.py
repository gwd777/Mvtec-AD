...
class FaissNN(object):
    def __init__(self, on_gpu: bool = True, num_workers: int = 4) -> None:
    ...
    def run(
        ...
        Args:
            query_features: Features to retrieve.                       # 查询特征：要检索的特征。
...
class NearestNeighbourScorer(object):
    def __init__(self, n_nearest_neighbours: int, nn_method=FaissNN(False, 4)) -> None:
        ...
        # Python当中的匿名函数；这个lambda函数接受一个参数 query，并调用 self.nn_method.run 方法，其中 n_nearest_neighbours 是传递给 run 方法的参数之一

    ...
    for class_name in mvtec.CLASS_NAMES:
        ...
        if not os.path.exists(train_feature_filepath):
            ...
            for i in range(H * W):
                ...
                hist = calculate_histogram2(numpy_value, bins=C)
                images_i_hist.append(hist)
        ...
        for samplex in embedding_vectors:
            image_i_scores = []
            for i in range(H * W):
                T_hist = featuremap_i_hist[i]
                sample_i = samplex[:, i]
                Test_hist = calculate_histogram2(sample_i, bins=C)
                c_score = compare_histograms(Test_hist, T_hist, method=cv2.HISTCMP_CORREL)
        ...
        threshold_value = 0.3
        binary_scores = np.where(scores >= threshold_value, 0, 1)       # 二值化处理

        img_scores = []
        evaluation_threshold = 0.1
        for img in binary_scores:
            proportion_of_ones = np.sum(img) / (binary_scores.shape[1] * binary_scores.shape[2])        # 计算1的比例
            result = 1.0 if proportion_of_ones >= evaluation_threshold else 0.0
            img_scores.append(result)
